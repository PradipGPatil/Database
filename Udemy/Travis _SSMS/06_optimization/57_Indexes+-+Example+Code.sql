...
-- if we see below we have joined by salesorderid
...
(
SalesOrderID INT,
-- as sales order id has many join which mean it does not have unisqu value 
...
-- if we see below we are joining on basis on product id so created non clustered index on productid
